...
output SSH_connections {
  value = [
    for instance in oci_core_instance.tf-demo03-instances.*:
      "${var.instances_name[index(oci_core_instance.tf-demo03-instances.*, instance)]}: ssh -i ${var.ssh_private_key_file} opc@${instance.public_ip}"
  ]
}
